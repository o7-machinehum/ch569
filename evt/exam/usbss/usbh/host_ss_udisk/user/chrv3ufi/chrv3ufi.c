...
//uint8_t	CHRV3BulkOnlyCmd( uint8_t * DataBuf )
//{
...
*/
//uint8_t R8_USB_DEV_AD;
//uint8_t R8_USB_MIS_ST;
//uint8_t R8_UHOST_CTRL;
//uint8_t R8_USB_INT_FG;
uint8_t RB_UMS_DEV_ATTACH;
uint8_t RB_UMS_SUSPEND;
...
/* 如果指定枚举序号CHRV3vFileSize为0xFFFFFFFF后调用FileOpen，那么每搜索到一个文件FileOpen都会调用本回调程序，
   ...
   如果在本回调程序xFileNameEnumer中将CHRV3vFileSize置为1，那么可以通知FileOpen提前结束搜索。以下是回调程序例子 */
	...
	FileCount = (uint16_t)( 0xFFFFFFFF - CHRV3vFileSize );  /* 当前文件名的枚举序号,CHRV3vFileSize初值是0xFFFFFFFF,找到文件名后递减 */
...
//	pTX_DMA_A_REG = (uint32_t *)&R16_UH_TX_DMA;  /* 指向发送DMA地址寄存器,由应用程序初始化 */
//	pRX_DMA_A_REG = (uint32_t *)&R16_UH_RX_DMA;  /* 指向接收DMA地址寄存器,由应用程序初始化 */
//	pTX_LEN_REG = (uint16_t *)&R8_UH_TX_LEN;  /* 指向发送长度寄存器,由应用程序初始化 */
//	pRX_LEN_REG = (uint16_t *)&R8_USB_RX_LEN;  /* 指向接收长度寄存器,由应用程序初始化 */
