...
/* Global Variable */
extern volatile uint8_t USB30_rec_flag;
extern volatile uint8_t Link_Sta;

extern volatile uint32_t Endp1_Up_LastPackNum;                                            /* The number of packets uploaded by endpoint 1 */
extern volatile uint32_t Endp1_Up_LastPackLen;                                            /* The length of the last packet uploaded by endpoint 1 */
extern volatile uint32_t Endp1_Up_Status;                                                 /* Endpoint 1 upload status: 0: free; 1: Uploading; */
extern volatile uint32_t Endp1_Down_LastPackNum;                                          /* The number of packets last downloaded by endpoint 1 */
extern volatile uint32_t Endp1_Down_LastPackLen;                                          /* The length of the last packet transmitted by endpoint 1 */
extern volatile uint32_t Endp1_Down_Status;                                               /* Endpoint 1 download status: 0: download; 1: Stop downloading; */
extern volatile uint32_t Endp1_Down_IdleCount;                                            /* Endpoint 1 downlink idle timing; */
extern volatile uint8_t  USB_Down_StopFlag;                                               /* USB Download pause flag(USB Received a non-full packet,must wait until the HSPI is sent before continuing to download) */
extern volatile uint8_t  HSPI_RX_StopFlag;                                                /* HSPI Receive pause flag(HSPI Received a non-full packet,You must wait for the USB upload to complete before continuing to receive) */
