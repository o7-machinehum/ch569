...
void USB20_Endp_Init( void )
{
    ...
    R32_UEP1_TX_DMA = (uint32_t)(uint8_t *)DEF_ENDP1_TX_DMA_ADDR;
	R32_UEP1_RX_DMA = (uint32_t)(uint8_t *)DEF_ENDP1_RX_DMA_ADDR;
		...
		switch( end_num )  /*Endpoint number*/
		{
		   ...
		   case 1:
		       if( rx_token == PID_IN )
		       {
                   ...
                   R32_UEP1_TX_DMA = (uint32_t)(uint8_t *)HSPI_Rx_Data_DealAddr;
