...
* Version            : V1.00
* Date               : 2021/08/01
* Description        : Simulated U-disk (EMMC as storage medium) main program
*******************************************************************************
...
/******************************************************************************/
/* MODE_SENSE response data for CMD 0x1A */
...
/******************************************************************************/
/* MODE_SENSE response data for CMD 0x5A */
...
volatile uint8_t  Udisk_Status = 0x00;                                                              /* U-disk enable status */
volatile uint8_t  Udisk_Transfer_Status = 0x00;                                                     /* U-disk transfer status */
volatile uint32_t Udisk_Capability = 0x00;                                                          /* U-disk formatted capacity */
...
* Description    : Set U-disk command execution status
* Input          : key------Sense key for error details
*                  asc------Additional sense code for error details
...
* Description    : Handle U-disk command execution failure
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
* Description    : Prepare for read/write command processing
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
{
    /* Save the current sector LBA address */
    ...
    /* Save the current transfer data length */                    
...
void UDISK_SCSI_CMD_Deal( void )
{
      ...
      &&( mBOC.mCBW.mCBW_Sig[ 2 ] == 'B' ) && ( mBOC.mCBW.mCBW_Sig[ 3 ] == 'C' ) )
    {
        ...
        /* U-disk SCSI command processing */ 
...
* Description    : U-disk IN endpoint handler
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
* Description    : U-disk OUT endpoint handler
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
{
    uint32_t i;
    /* Handle endpoint OUT data transfer */
    if( Udisk_Transfer_Status & DEF_UDISK_BLUCK_DOWN_FLAG )
    {
//        /* Process one OUT USB data packet */
//        UDISK_Down_OnePack( );
        UDISK_OutPackflag = 1;
    }
    else
...
* Description    : Upload data packet via bulk IN endpoint 2
* Input          : Transfer_DataLen--- Remaining transfer data length
*                  *pBuf---Data buffer pointer
* Output         : Transfer_DataLen--- Updated remaining data length
*                  *pBuf---Updated data buffer pointer
* Return         : None
*******************************************************************************/
...
{                                            
    ...
    /* Load data into upload buffer and send */
    if( Link_Sta == LINK_STA_1 )
    {
...
* Description    : Upload CSW (Command Status Wrapper) via bulk IN endpoint 2
* Input          : CBW_Tag_Save---Command tag
*                  CSW_Status---Current command execution result status
* Output         : Bit_DISK_CSW---CSW pending upload flag
*                  Bit_DISK_Bluck_Up---Bulk IN transfer flag
*                  Bit_DISK_Bluck_Down---Bulk OUT transfer flag
* Return         : None
*******************************************************************************/
...
*
* Description    : Process bulk endpoint data packets - disables USB interrupt for speed, handles EMMC and USB transfers simultaneously
*
* Return         : None
*******************************************************************************/
...
* Description    : USB mass storage device upload one data packet.
*                  Used during U-disk read operations; after each packet upload,
*                  checks whether to continue uploading.
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
{
    /* Check if sector read operation is needed */
    uint8_t *pbuf = NULL;
    uint16_t preqnum = 0 ;
    uint8_t s;
    ...
    /* Check if all data has been uploaded */    
...
* Description    : USB mass storage device process one OUT data packet
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
